...
if conn:
    ...
    try:
        ...
        jackdb02_ami_id = conn.register_image(name=ami_name, description=ami_desc, architecture=ami_arch,
                            root_device_name=ami_root, block_device_map=ami_bkdm,
    ...
    if as_conn:
        ...
        new_lc = LaunchConfiguration(name=lc_name, image_id=lc_imag, key_name=lc_keyn, security_groups=lc_secg,
                    instance_type=lc_inst, ramdisk_id=lc_ramd,
					#instance_type=lc_inst, kernel_id=lc_kern, ramdisk_id=lc_ramd,
        ...
        as_group.health_check_period = 300
        try:
            as_group.update()
        except:
